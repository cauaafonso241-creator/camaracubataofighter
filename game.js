...
function renderRoster(){
  ...
  $("#selectedCard").innerHTML=`<div class="big-avatar">${avatar(f)}</div>
    <div class="card-info"><small>LUTADOR SELECIONADO</small><h3>${f.name}</h3>
    <p>ESPECIAL: <b>${f.power}</b></p>
...
function startGame(){
  ...
  state={p1:{hp:100,en:0,x:15,wins:0,block:false,cool:0,stun:0},p2:{hp:100,en:0,x:75,wins:0,block:false,cool:0,stun:0},round:1,time:75,over:false};
  ...
  resetRound(true);loop=setInterval(tick,50);
  clock=setInterval(()=>{if(!paused&&!state.over){state.time--;$("#timer").textContent=state.time;if(state.time<=0)endRound()}},1000)
}
...
function specialVisual(id,f){
  const stage=$("#stage"),fx=document.createElement("div");fx.className="special-object "+f.type;
  fx.textContent={ball:"⚽",cavaco:"🎸",dogs:"🐕 🐕",scissors:"✂️",water:"💧",process:"📁",dumbbell:"🏋️",sonic:")))",hair:"〰",kick:"💥",kickbox:"🥊",spin:"🌀",power:"⚡"}[f.type]||"⚡";
  fx.style.left=(state[id].x+8)+"%";fx.style.bottom="31%";if(id==="p2")fx.classList.add("reverse");
  stage.appendChild(fx);setTimeout(()=>fx.remove(),650);announce(f.power+"!",650)
}
function attack(att,def,type){
  if(state.over||paused||att.cool>0||att.stun>0)return;
  const aid=att===state.p1?"p1":"p2",did=def===state.p1?"p1":"p2";let range=Math.abs(att.x-def.x),dmg=type==="special"?22:type==="kick"?10:6;
  if(type==="special"&&att.en<50)return;att.cool=type==="special"?12:5;
  if(type==="special"){att.en-=50;anim(aid,"specialFx",500);specialVisual(aid,fighters[aid==="p1"?selected:enemy]);range=0}
  else anim(aid,type==="kick"?"kick":"attack");
  if(range<(type==="kick"?20:17)||type==="special"){if(def.block)dmg=Math.ceil(dmg*.25);else def.stun=type==="special"?7:3;def.hp=Math.max(0,def.hp-dmg);att.en=Math.min(100,att.en+(type==="special"?3:10));anim(did,"hit");if(def.hp<=0)setTimeout(endRound,250)}
  updateHUD()
}
...
function tick(){
  if(!state||state.over||paused)return;const p=state.p1,e=state.p2;
  ...
  if(!p.stun&&keys.a)p.x=Math.max(2,p.x-1.25);if(!p.stun&&keys.d)p.x=Math.min(84,p.x+1.25);
  ...
  const dist=Math.abs(e.x-p.x);e.block=!e.stun&&Math.random()<.035;$("#p2").classList.toggle("block",e.block);
  const aiMoving=!e.stun&&dist>15;$("#p2").classList.toggle("walking",aiMoving);
  if(aiMoving)e.x+=e.x>p.x?-.72:.72;else if(!e.stun&&Math.random()<.075)attack(e,p,e.en>=50&&Math.random()<.2?"special":Math.random()<.45?"kick":"punch");
  updateHUD()
}
addEventListener("keydown",e=>{const k=e.key.toLowerCase();if(["a","d","w","s","j","k","l"].includes(k)){keys[k]=true;e.preventDefault()}if(k==="w"&&state&&!state.over)anim("p1","jump",450);if(e.key==="Escape")togglePause()});
